...
class UntappdAuthenticator {
    
    static let sharedInstance = UntappdAuthenticator()
    ...
    func authorizeWithToken(token: String) {
        ...
        Alamofire.request(authorizeURL, parameters: parameters).responseJSON { response in
            ...
            do {
                ...
                UserDefaults.standard.setValue(authResponse.accessToken, forKey: .accessToken)
                NotificationCenter.default.post(key: .untappdAuthorizationSuccessful)
            } catch {
                NotificationCenter.default.post(key: .untappdAuthorizationFailed)
            }
        }
